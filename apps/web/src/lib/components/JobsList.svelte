      ...
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {#each jobs as job}
          <button
            ...
            class="block p-3 sm:p-4 rounded-xl hover:bg-[var(--bg)] border border-[var(--border)] shadow-sm transition-all text-left"
          >
            <div class="flex justify-between items-start mb-2">
              <p class="text-xs sm:text-sm truncate font-semibold flex-1 mr-2 text-[var(--fg)]">{job.youtubeUrl}</p>
